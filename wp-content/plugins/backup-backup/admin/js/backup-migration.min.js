    ...
    bmi: {
      requestWakeLock: async () => {
        try {
          if ( wakeLock == null && 'wakeLock' in navigator) {
            ...
            // Listen for release (e.g., if user switches tabs)
        ...
        return new Promise(async function(resolve, reject) {
          ...
          try {
            ...
            if (res.ok === true) {
              ...
              throw new Error('Response is not OK (' + res.status + '): ' + (response || 'No response body'));
      ...
      tooltips: {
        ...
        init: function() {
          ...
          function extractThemesFromElement(el) {
            try {
              const base = settings.theme;
              const classList = (el && el.className ? el.className : '')
                .split(/\s+/)
                .filter(Boolean);
              const extracted = classList.filter(c => c.indexOf('bmi-tt-') === 0);
              if (extracted.length) return base.concat(extracted);
              else return base;
            } catch (e) {
          ...
          for (let i = 0; i < tts.length; ++i) {
            ...
            s.theme = extractThemesFromElement(tooltip);
          ...
          for (let i = 0; i < premiums.length; ++i) {
            ...
            semiums.theme = extractThemesFromElement(premium);
      ...
      fillWithNewBackups: function(res, ongoing, cb = function() {}, search = '') {
        // let backupsList = {};
        ...
        for (let name in local) {
          if (search !== '' && local[name][0].toLowerCase().indexOf(search.toLowerCase()) === -1) continue;
          let backup = local[name];
          backups[backup[7]] = backup;
          backups[backup[7]][10] = backup[9];
        ...
        for (let drive in external) {
          for (let backupKey in external[drive]) {
            if (search !== '' && external[drive][backupKey][0].toLowerCase().indexOf(search.toLowerCase()) === -1) continue;
        ...
        for (; i < backupsSorted.length; ++i) {
          ...
          if (typeof backups[index][8] != 'undefined') {
            if (typeof backups[index][8] == 'object') {
              for (let storage in backups[index][8]) {
                ...
                } else if (storage == 'pcloud') {
                  isPCloud = backups[index][8][storage];
                } else if (storage == 'aws') {
                  isAWS = backups[index][8][storage];
                } else if (storage == 'wasabi') {
                  isWasabi = backups[index][8][storage];
                } else if (storage == 'backupbliss') {
                  isBackupBliss = backups[index][8][storage];
                } else if (storage == 'sftp') {
                  isSftp = backups[index][8][storage];
                }
              }
            }
          }

          isCloud = isGoogleDrive || isFtp || isDropbox || isOneDrive || isBackupBliss || isSftp || isAWS || isWasabi || isPCloud;
          ...
          if (isPCloud) $template.attr('pcloud-id', isPCloud);

          if (isAWS) $template.attr('aws-id', isAWS);

          if (isWasabi) $template.attr('wasabi-id', isWasabi);
          if (isBackupBliss) $template.attr('backupbliss-id', isBackupBliss);

          if (isSftp) $template.attr('sftp-id', isSftp);

          $template.find('.br_label').attr('for', id);
          $template.find('.br_checkbox').attr('id', id);
          $template.find('.bc-download-btn').attr('href', $('#BMI_BLOG_URL').text().trim() + '/?backup-migration=BMI_BACKUP&bmi-id=' + name + '&sk=' + $('#BMI_SECRET_KEY').text().trim());
          $template.find('.bc-logs-btn').attr('href', $('#BMI_BLOG_URL').text().trim() + '/?backup-migration=BMI_BACKUP_LOGS&bmi-id=' + name + '&sk=' + $('#BMI_SECRET_KEY').text().trim());
          ...
          if (isOneDrive) {
            ...
            storages.push('OneDrive');
          } else {
            $template.find('.br_stroage').find('.strg-onedrive').addClass('img-red');
          }

          if (isPCloud) {
            $template.find('.br_stroage').find('.strg-pcloud').addClass('img-green');
            storages.push('pCloud');
          } else {
            $template.find('.br_stroage').find('.strg-pcloud').addClass('img-red');
          }

          if (isAWS) {
            $template.find('.br_stroage').find('.strg-aws').addClass('img-green');
            storages.push('AWS S3');
          } else {
            $template.find('.br_stroage').find('.strg-aws').addClass('img-red');
          }

          if (isWasabi) {
            $template.find('.br_stroage').find('.strg-wasabi').addClass('img-green');
            storages.push('Wasabi');
          } else {
            $template.find('.br_stroage').find('.strg-wasabi').addClass('img-red');
          }
          if (isBackupBliss) {
            $template.find('.br_stroage').find('.strg-backupbliss').addClass('img-green');
            storages.push('BackupBliss');
          } else {
            $template.find('.br_stroage').find('.strg-backupbliss').addClass('img-red');
          }

          if (isSftp) {
            $template.find('.br_stroage').find('.strg-sftp').addClass('img-green');
            storages.push('SFTP');
          } else {
            $template.find('.br_stroage').find('.strg-sftp').addClass('img-red');
          }

          if (isLocalBackup) {
            $template.find('.br_stroage').find('.strg-local').addClass('img-green');
            storages.push('Local Storage');
            for (let key in backups[index][10]) {
              if (backups[index][10].hasOwnProperty(key)) {
                if ($template.find('.br_stroage').find('.strg-' + key).length > 0 && $template.find('.br_stroage').find('.strg-' + key).hasClass('img-red')) {
                  let storageElement = $template.find('.br_stroage').find('.strg-' + key);
                  let storageElementTooltip = storageElement.attr('tooltip');

                  storageElement.addClass('can-be-manually-uploaded');
                  storageElement.attr('tooltip', 'Upload To ' + storageElementTooltip);
                }
              }
            }
          } else {
          ...
          isDirectLinkEnabled = $('[name="radioAccessViaLink"]:checked').val() === "true";

          if (!isDirectLinkEnabled) {
            $template.find('.bc-url-btn svg')
              .css({ 
                cursor: "not-allowed",
                color: "red"
              })
              .addClass('disabled');
            $template.find('.bc-url-btn')
              .attr('tooltip', $('#direct-link-disabled-tooltip').text().trim());
          } else {
            $template.find('.bc-url-btn svg')
              .css({ 
                cursor: "pointer",
            ...
            $template.find('.bc-url-btn')
              .attr('tooltip', $('#direct-link-enabled-tooltip').text().trim());
          }
      ...
      fillOnGoing: function(ongoing = false) {
            ...
            if (ongoing.current_upload.length == 0) {
              ...
              switch (currentUploadType) {
                ...
                case 'dropbox':
                  ...
                  break;
                case 'onedrive':
                  externalStorageType = 'OneDrive';
                  break;
                case 'aws':
                  externalStorageType = 'Amazon S3';
                  break;
                case 'wasabi':
                  externalStorageType = 'Wasabi';
                  break;
                case 'pcloud':
                  externalStorageType = 'pCloud';
                case 'backupbliss':
                  externalStorageType = 'BackupBliss';
                  break;
                case 'sftp':
                  externalStorageType = 'SFTP';
                  break;
              }
        ...
        if (typeof ongoing.failed != 'undefined') {
          ...
          for (let token in ongoing.failed) {
            ...
            if (md5) {
              ...
              if (uploadType == 'pcloud') externalStorageType = 'pCloud';
              if (uploadType == 'onedrive') externalStorageType = 'OneDrive';
              if (uploadType == 'aws') externalStorageType = 'AWS';
              if (uploadType == 'wasabi') externalStorageType = 'Wasabi';
              if (uploadType == 'backupbliss') externalStorageType = 'BackupBliss';
              if (uploadType == 'sftp') externalStorageType = 'SFTP';
              message += externalStorageType;
        ...
        // Update tooltips of external storage icons with upload status
        const tooltips = {
          'img-green': 'Backup is available on %s.',
          'img-orange': 'Upload to %s is in progress.',
          'img-purple': 'Upload to %s is queued.',
          'img-red': 'Backup is not available on %s.',
        };

        const $tbody = $('#bmi_restore_tbody');
        let storagesToStr = {
          'strg-local': 'Local Storage',
          'strg-ftp': 'FTP',
          'strg-aws': 'AWS S3',
          'strg-backupbliss': 'BackupBliss',
          'strg-sftp': 'SFTP',
          'strg-gdrive': 'Google Drive',
          'strg-onedrive': 'OneDrive',
          'strg-pcloud': 'pCloud',
          'strg-wasabi': 'Wasabi',
          'strg-dropbox': 'Dropbox',
        }

        Object.entries(tooltips).forEach(([imgClass, template]) => {
          $tbody.find(`.${imgClass}`).each(function () {
            const $el = $(this);
            const classes = $el.attr('class').split(/\s+/);
            if (classes.includes('can-be-manually-uploaded')) return;
            const strgClass = classes.find(c => c.startsWith('strg-'));

            if (!strgClass || ['strg-suc', 'strg-warn', 'strg-ong', 'strg-wait'].includes(strgClass)) return;
              
            let cap = '';
            cap = storagesToStr[strgClass] || strgClass.replace('strg-', '').charAt(0).toUpperCase() + strgClass.replace('strg-', '').slice(1);
            const tooltip = template.replace('%s', cap);
            $el.tooltipster('content', tooltip);
          });
        });
      },

      adjustStorageIcons: function() {
        let es = ['gdrive', 'onedrive', 'sftp', 'ftp', 'dropbox', 'aws', 'wasabi', 'pcloud'];

        let activeEsCount = 2;
        for (let i = 0; i < es.length; ++i) {
          if ($('#bmi-pro-storage-' + es[i] + '-toggle').is(':checked')) {
            $('.strg-' + es[i]).show();
            activeEsCount++;
          } else {
            $('.strg-' + es[i]).hide();
        ...
        if ($('.storage-icons-container').length == 0) return;
        if (activeEsCount > 5 ) {
          $('.storage-icons-container').css('grid-template-rows', 'repeat(2, auto)');
        } else {
          $('.storage-icons-container').css('grid-template-rows', 'none');
        }
      },
      ...
      searchInBackups: function(keyword, callagain = function() {}) {
        $.bmi.getCurrentBackups(function(res) {
          $.bmi.fillWithNewBackups(res.backups, res.backups.ongoing, function() {
            callagain();
          }, keyword );
    ...
    init: function() {
      ...
      function handleSelect(select) {
        ...
        if (select.style.display != 'none') {
          ...
          let fixedWidth = $select.attr('data-width');
          if (fixedWidth) width = parseInt(fixedWidth);
          $(dropdown).find('.dropdown-title')[0].style.minWidth = (width + 10) + 'px';
          let readonly = $select.attr('data-readonly');
          if (readonly == 'true') {
            $(dropdown).find('.dropdown-title')[0].style.pointerEvents = 'none';
            $(dropdown).closest('.bmi-dropdown')[0].style.background = '#f1f1f1';
          }
...
jQuery(document).ready(function ($) {
  ...
  $("#bmi_restore_tbody").on("click", ".bc-url-btn", function (e) {
    e.preventDefault();
    let $el = e.target;
    if ($el.closest("svg").classList.contains("disabled")) return;
  ...
  function saveBtnEventHandler(e = false, type = false, cb = () => { }) {
    ...
    if (save == "save-storage") {
      ...
      data["sftp"] = $("#bmi-pro-storage-sftp-toggle").is(":checked") === true ? 'true' : 'false';
      ...
      data['pcloud'] = $('#bmi-pro-storage-pcloud-toggle').is(':checked') === true ? 'true' : 'false';
      ...
      data['aws'] = $('#bmi-pro-storage-aws-toggle').is(':checked') === true ? 'true' : 'false';
      data['wasabi'] = $('#bmi-pro-storage-wasabi-toggle').is(':checked') === true ? 'true' : 'false';
      ...
      data["use_new_search_replace_engine"] = isChecked("bmi-use-new-search-replace-engine"); // OTHER::NEW_SEARCH_REPLACE_ENGINE
      data["use_new_database_export_engine"] = isChecked("bmi-use-new-database-export-engine"); // OTHER::NEW_DATABASE_EXPORT_ENGINE
    ...
    $.bmi
      ...
      .then(function (res) {
        if (res.status == "success") {
          if (res.errors <= 0) {
            if ($('.save-brw-details').length > 0) $('.save-brw-details').triggerHandler('click');
  ...
  function refreshContentInstant(cb = function () { }) {
    ...
    httpGet(
      url +
      "?backup-migration=PROGRESS_LOGS&progress-id=latest_full.log&bmi-id=current&t=" +
      +new Date() +
      "&sk=" +
    ...
    setTimeout(function () {
      ...
      $.bmi
        ...
        .then(function (res_latest) {
          let url =
            url_x +
            "?backup-migration=BMI_BACKUP&bmi-id=" +
            res_latest +
            "&t=" +
            +new Date() +
            "&sk=" +
          ...
          let logs =
            url_x +
            "?backup-migration=PROGRESS_LOGS&progress-id=latest.log&bmi-id=current&t=" +
            +new Date() +
            "&sk=" +
            $("#BMI_SECRET_KEY").text().trim();
    ...
    isMigrationLocked(function (isNotLocked) {
      if (isNotLocked) {
        ...
        if (e.target.closest('tr').getAttribute('data-is-local') === 'no') {
          ...
          if(e.target.closest('tr').getAttribute('pcloud-id') !== null){
            name = '?#pcloud#_' + e.target.closest('tr').getAttribute('pcloud-id');
            current_restore = name;
          }

          if (e.target.closest('tr').getAttribute('aws-id') !== null) {
            name = '?#aws#_' + e.target.closest('tr').getAttribute('aws-id');
            current_restore = name;
          }

          if (e.target.closest('tr').getAttribute('wasabi-id') !== null) {
            name = '?#wasabi#_' + e.target.closest('tr').getAttribute('wasabi-id');
            current_restore = name;
          }
          
          if (e.target.closest('tr').getAttribute('backupbliss-id') !== null) {
            name = '?#backupbliss#_' + e.target.closest('tr').getAttribute('backupbliss-id');
            current_restore = name;
          }

          if (e.target.closest('tr').getAttribute('sftp-id') !== null) {
            name = '?#sftp#_' + e.target.closest('tr').getAttribute('sftp-id');
            current_restore = name;
          }
  ...
  $('#restore-start-sure').on('click', function () {
    ...
    if ($('#restore-ok').is(':checked')) {
      ...
      else if (current_restore.includes('?#pcloud#_')) downloadPCloud();
      else if (current_restore.includes('?#aws#_')) downloadS3(true, 'aws');
      else if (current_restore.includes('?#wasabi#_')) downloadS3(true, 'wasabi');
      else if (current_restore.includes('?#backupbliss#_')) downloadBackupBliss();
      else if (current_restore.includes('?#sftp#_')) downloadSFTP();
      else letsRestore();
    } else
  ...
  function getMigrationLogs(cb = function () { }) {
    ...
    httpGet(
      url +
      "?backup-migration=PROGRESS_LOGS&progress-id=latest_migration_full.log&bmi-id=current&t=" +
      +new Date() +
      "&sk=" +
      $("#BMI_SECRET_KEY").text().trim(),
      function (res1) {
  ...
  function downloadBackupBliss(startRestoreProcess = true) {
    $("#restore-live-log-wrapper").find("pre")[0].innerText = "";
    let currentBackupBlissIdRestoration =
      current_restore.split("?#backupbliss#_")[1];

    let md5 = $("#bmi_restore_tbody").find("tr[backupbliss-id='" + currentBackupBlissIdRestoration + "']").attr("md5");
    current_restore = null;
      ...
      resetLogs(true, function () {
        fixHtaccess(function () {
          if (isNotLocked) {
            ...
            performBackupBlissDownload(startRestoreProcess, currentBackupBlissIdRestoration, 0, false, md5);
          }
        });
      });
    });
  ...
  $.bmi.downloadBackupBliss = downloadBackupBliss;
  ...
  function downloadS3(startRestoreProcess = true, provider = 'aws') {
    $("#restore-live-log-wrapper").find("pre")[0].innerText = "";
    let currentS3IdRestoration = current_restore.split("?#" + provider + "#_")[1];
    let md5 = $("#bmi_restore_tbody").find("tr[" + provider + "-id='" + currentS3IdRestoration + "']").attr("md5");
    current_restore = null;
      ...
      resetLogs(true, function () {
        fixHtaccess(function () {
          if (isNotLocked) {
            ...
            performS3Download(startRestoreProcess, provider, currentS3IdRestoration, 0, false, md5);
          }
        });
      });
    });
  ...
  function performS3Download(startRestoreProcess = true, provider = 'aws', fileId = false, step = 0, size = false, md5 = false, originalFilename = false, writepath = false, chunksize = 1000000, secret = false) {
    $.bmi.ajax('download-cloud-backup', {
      storage: 's3',
      provider: provider,
      fileId: fileId,
      ...
      if (res.status == 'success') {
        ...
        if (typeof res.finished != 'undefined' && res.finished != 'true') {
          step++;
          performS3Download(startRestoreProcess, provider, fileId, step, size, md5, originalFilename, writepath, chunksize, secret);
        } else if (res.finished == 'true') {
          cli_quickmigration = true;
          current_restore = res.filename;
          refreshLogAndProgressRestore(false);
  ...
  function performBackupBlissDownload(startRestoreProcess = true,fileId = false, step = 0, size = false, md5 = false, originalFilename = false, writepath = false, chunksize = 1000000, secret = false) {
    ...
    $.bmi.ajax('download-cloud-backup', {
      ...
      if (res.status == 'success') {
        ...
        if (typeof res.finished != 'undefined' && res.finished != 'true') {
          ...
          step++;
          performBackupBlissDownload(startRestoreProcess, fileId, step, size, md5, originalFilename, writepath, chunksize, secret);
  ...
  function downloadSFTP(startRestoreProcess = true) {
    console.log("downloadSFTP");
    $("#restore-live-log-wrapper").find("pre")[0].innerText = "";
    let currentSFTPIdRestoration = current_restore.split("?#sftp#_")[1];
    let md5 = $("#bmi_restore_tbody").find("tr[sftp-id='" + currentSFTPIdRestoration + "']").attr("md5");
    current_restore = null;
      ...
      resetLogs(true, function () {
        fixHtaccess(function () {
          if (isNotLocked) {
            ...
            performSFTPDownload(startRestoreProcess, currentSFTPIdRestoration, 0, false, md5);
          }
        });
      });
    });
  ...
  function performSFTPDownload(startRestoreProcess = true, fileId = false, step = 0, size = false, md5 = false, originalFilename = false, writepath = false, chunksize = 1000000, secret = false) {

    $.bmi.ajax('download-sftp-backup', {
      ...
      if (res.status == 'success') {
        ...
        if (typeof res.finished != 'undefined' && res.finished != 'true') {
          ...
          step++;
          performSFTPDownload(startRestoreProcess, fileId, step, size, md5, originalFilename, writepath, chunksize, secret);
  ...
  $.bmi.downloadSFTP = downloadSFTP;



  function downloadPCloud(startRestoreProcess = true) {
    $("#restore-live-log-wrapper").find("pre")[0].innerText = "";
    let currentPCloudIdRestoration = current_restore.split("?#pcloud#_")[1];
    let md5 = $("#bmi_restore_tbody").find("tr[pcloud-id='" + currentPCloudIdRestoration + "']").attr("md5");
    current_restore = null;
      ...
      resetLogs(true, function () {
        fixHtaccess(function () {
          if (isNotLocked) {
            ...
            performPCloudDownload(startRestoreProcess, currentPCloudIdRestoration, 0, false, md5);
          }
        });
      });
    });
  ...
  $.bmi.downloadPCloud = downloadPCloud;

  function performPCloudDownload(
    ...
    $.bmi
      ...
      .then(function (res) {
        if (res.status == "success") {
          if (typeof res.size != "undefined") size = res.size;
          if (typeof res.md5 != "undefined") md5 = res.md5;
          if (typeof res.originalFilename != "undefined")
      ...
      function (isNotLocked) {
        if (isNotLocked) {
          setTimeout(function () {
            ...
            // httpGet(url + '?backup-migration=PROGRESS_LOGS&progress-id=latest_migration.log&bmi-id=current&t=' + +new Date(), function(res) {
            //
            //   if (res == false) {
            //     setTimeout(function () {
            //       restoreFailed();
      ...
      .then(function (res) {
        if (res.status === "cli") {
          ...
          if (typeof res.options.dbFinished != "undefined") {
            ...
            if (
              (res.options.step == 6 || res.options.step == "6") &&
              typeof res.options.repeat_restore != "undefined"
            ) {
              if (
                res.options.repeat_restore === true ||
                res.options.repeat_restore === "true" ||
                res.options.repeat_restore === "1"
              ) {
                res.options.step = 5;
                res.options.firstFileRestore = "false";
              }
            }
          }
  ...
  $(".bmi-review-btn").on("click", function (e) {
    e.preventDefault();

    var url = $(this).attr("href");

    $.bmi.ajax("clicked-on-plugin-review", {}).then(function (res) {
      $(".bmi-ask-for-review").hide(300);
  ...
  $(".site-reloader").on("click", function () {
    ...
    let url_final =
      url +
      "?backup-migration=AFTER_RESTORE&bmi-id=" +
      autoLog.l +
  ...
  function refreshBBStorageInfo(silent=false) {
    $('.refresh-img').addClass("loading");
    $.bmi.ajax('bb-storage-info').then(function (res) {

      if (res.status == 'success') {
        $(".bb-storage-amount").text(res.data.storage_info.total_space_humanized);
        $(".bb-storage-used").text(res.data.storage_info.used_space_humanized);
        $(".bb-storage-used-percent").text("("+res.data.storage_info.used_space_percent+"%)");
      } else {
        if (!silent)
          $.bmi.alert('error', res.message);
      ...
      $('.refresh-img').removeClass("loading");
      if (!silent)
        $.bmi.alert('error', 'There was an error contacting the plugin backend.');

    });
  }

  refreshBBStorageInfo(true);

  $(".refresh-bb-storage").on("click", function(e){
    e.preventDefault();
    refreshBBStorageInfo();
  });

  $("#bb-upload-fail-dismiss").on("click", function(e){
    e.preventDefault();
    $.bmi
      .ajax("dismiss-error-notice", {
        option_id: "backupbliss-dismiss-upload-issue",
      })
      .then(function (res) {})
      .catch(function (error) {});
      
    $("#bb-warning-notice").hide(300);
    setTimeout(function () {
      $("#bb-warning-notice").remove();
    }, 330);
  });

  $(".bb-disconnect").on("click", function (e) {
    e.preventDefault();
    $.bmi.modal("bb-disconnect-modal").open();
  });

  $(".bb-disconnect-cancel").on("click", function (e) {
    e.preventDefault();
    $.bmi.modal("bb-disconnect-modal").close();
  });

  $(".bb-disconnect-btn").on("click", function(e){
    e.preventDefault();
    $.bmi.modal("bb-disconnect-modal").close();
    $.bmi.modal("freeze-loading-modal").open();
    $.bmi.ajax('bb-disconnect').then(function (res) {

      if (res.status == 'success') {
        $(".how-it-works").show();
        $(".youre-connected").hide();
      } else {
  ...
  $(".bb-connect").on("click", function(e){
    e.preventDefault();
    $.bmi.modal("freeze-loading-modal").open();
    $.bmi.ajax('bb-connect', {
  ...
  $('#dropbox-connect-btn').on('click', function (e) {

    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    $.bmi.ajax('get-dropbox-token').then(function (res) {

      $.bmi.modal('freeze-loading-modal').close();
      if (typeof res.token != 'undefined') {

        let url = 'https://authentication.backupbliss.com/v1/dropbox/connect';
        url += '?token=' + encodeURIComponent(res.token);
        url += '&redirect=' + encodeURIComponent(window.location.origin + window.location.pathname);

        window.location.href = url;

      } else {

        $.bmi.alert('error', 'We could not generate individual token at this moment, please refresh page and try again [#Dropbox-02].');
      ...
      $.bmi.modal('freeze-loading-modal').close();
      $.bmi.alert('error', 'Cannot send request to your server, please check your internet connection [#Dropbox-01].');

    });

  });

  $('#dropbox-disconnect-btn').on('click', function (e) {

    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    $.bmi.ajax('disconnect-dropbox').then(function (res) {
      ...
      if (typeof res.status != 'undefined' && res.status == 'success') {
        ...
        $.bmi.alert('error', 'We could not disconnect you at this moment, please refresh page and try again.');
      ...
      $.bmi.modal('freeze-loading-modal').close();
      $.bmi.alert('error', 'Cannot send request to your server, please check your internet connection.');

    });

  });

  function verifyDropboxConnection(silent = false) {

      $.bmi.ajax('verify-dropbox-connection', { uri: window.location.host }).then(function (res) {

        $('#dropbox-unauthenticated-box').show();
        $('#dropbox-authenticated-box').hide();

        if (typeof res.status != 'undefined' && typeof res.result != 'undefined' && res.status == 'success') {

          if (res.result == 'connected') {

            $('#dropbox-unauthenticated-box').hide();
            $('#dropbox-authenticated-box').show();
          ...
          if (!silent) {
            $.bmi.alert('error', 'We could not verify Dropbox connection, some error happened during communication with API, look for more details in global logs [#DB-07].');
          }

        } else {
          if (!silent) {
            $.bmi.alert('error', 'We could not verify Dropbox connection, please refresh page and try again [#DB-05].');
          }
        ...
        if (!silent) {
          $.bmi.alert('error', 'Cannot send request to your server, please check your internet connection [#DB-08].');
        }

      });
  }

  $('#bmi-pro-storage-dropbox-toggle').on('change', function (e) {
      ...
      if ($('#bmi-pro-storage-dropbox-toggle').is(':checked') === true) {
  ...
  $('#bmip-dropbox-issues-dismiss').on("click", function () {
    $('#dropbox-issues').hide(300);
    setTimeout(function () {
      $('#dropbox-issues').remove();
    }, 330);
    $.bmi.ajax('dismiss-dropbox-notice', {}).then(function (res) { }).catch(function (error) { });
  });

  $('#gdrive-connect-btn').on('click', function (e) {

    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    let backupDirectoryPath = $('#bmip-googledrive-path').val();
    $.bmi.ajax('get-gdrive-token',
      ...
      if (typeof res.token != 'undefined') {
        ...
        let url = 'https://authentication.backupbliss.com/v1/gdrive/connect';
        url += '?token=' + encodeURIComponent(res.token);
        url += '&redirect=' + encodeURIComponent(window.location.origin + window.location.pathname);
        ...
        if (typeof res.status != 'undefined' && res.status === 'msg') {
          $.bmi.alert('warning', res.why);
        } else {
          $.bmi.alert('error', 'We could not generate individual token at this moment, please refresh page and try again [#GD-02].');
        }
      ...
      $.bmi.modal('freeze-loading-modal').close();
      $.bmi.alert('error', 'Cannot send request to your server, please check your internet connection [#GD-01].');

    });

  });

  $('#gdrive-disconnect-btn').on('click', function (e) {

    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    $.bmi.ajax('disconnect-gdrive').then(function (res) {
      ...
      if (typeof res.status != 'undefined' && res.status == 'success') {
        ...
        $.bmi.alert('error', 'We could not disconnect you at this moment, please refresh page and try again [#GD-03].');
      ...
      $.bmi.modal('freeze-loading-modal').close();
      $.bmi.alert('error', 'Cannot send request to your server, please check your internet connection [#GD-04].');

    });

  });

  function verifyGDriveConnection(silent = false) {

    $.bmi.ajax('verify-gdrive-connection', { uri: window.location.host }).then(function (res) {

      $('#gdrive-unauthenticated-box').show();
      $('#gdrive-authenticated-box').hide();

      if (typeof res.status != 'undefined' && typeof res.result != 'undefined' && res.status == 'success') {

        if (res.result == 'connected') {

          $('#gdrive-unauthenticated-box').hide();
          $('#gdrive-authenticated-box').show();
        ...
        if (!silent) {
          $.bmi.alert('error', 'We could not verify Google Drive connection, some error happened during communication with API, look for more details in global logs [#GD-07].');
        }

      } else {
        if (!silent) {
          $.bmi.alert('error', 'We could not verify Google Drive connection, please refresh page and try again [#GD-05].');
        }
      ...
      if (!silent) {
        $.bmi.alert('error', 'Cannot send request to your server, please check your internet connection [#GD-08].');
      }
  ...
  $('#bmi-pro-storage-gdrive-toggle').on('change', function (e) {
    ...
    if ($('#bmi-pro-storage-gdrive-toggle').is(':checked') === true) {
  ...
  $('#ftp-connect-btn').on('click', function (e) {
    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    let data = {};
    data['bmip-ftp-host'] = $('#bmip-ftp-host-ip').val();
    data['bmip-ftp-backup-dir'] = $('#bmip-ftp-backup-dir').val();
    data['bmip-ftp-host-port'] = $('#bmip-ftp-host-port').val();
    data['bmip-ftp-username'] = $('#bmip-ftp-user-name').val();
    data['bmip-ftp-password'] = $('#bmip-ftp-password').val();

    $.bmi.ajax('get-ftp-config', data).then(function (res) {

      $.bmi.modal('freeze-loading-modal').close();

      if (typeof res.status != 'undefined' && res.status === 'success') {
        if (e) $.bmi.alert('success', $('#bmi-save-connect-ftp').text(), 3000);
  ...
  $('#ftp-disconnect-btn').on('click', function (e) {

    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    $.bmi.ajax('disconnect-ftp').then(function (res) {
  ...
  $('#aws-connect-btn').on('click', function (e) {
    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    let data = {};
    data['access-key'] = $('#bmip-aws-access-key').val();
    data['secret-key'] = $('#bmip-aws-secret-key').val();
    data['bucket'] = $('#bmip-aws-bucket').val();
    data['sse'] = $('#bmip-aws-sse').is(':checked') === true ? $('#bmip-aws-sse').val().trim() : '';
    data['path'] = $('#bmip-aws-path').val();
    data['storage-class'] = $('[data-id="bmip-aws-storage-class"]').attr('data-selected');
    data['region'] = $('[data-id="bmip-aws-region"]').attr('data-selected');

    $.bmi.ajax('save-aws-config', data).then(function (res) {

      $.bmi.modal('freeze-loading-modal').close();

      if (typeof res.status != 'undefined' && res.status === 'success') {
        if (e) $.bmi.alert('success', $('#bmi-save-connect-s3-success').text(), 3000);
        window.location.reload();
      } else {
        $.bmi.alert('error', res.msg);
      }
      ...
      $.bmi.modal('freeze-loading-modal').close();
      $.bmi.alert('error', 'There was an error while trying to connect to Amazon S3, please try again.');
    });
  });


  $('#aws-disconnect-btn').on('click', function (e) {

    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    $.bmi.ajax('disconnect-aws').then(function (res) {
  ...
  function verifyAWSConnection(silent = false) {
    $.bmi.ajax('verify-aws-connection', { uri: window.location.host}).then(function (res) {
        $('#aws-unauthenticated-box').show();
        $('#aws-authenticated-box').hide();


        if (typeof res.status != 'undefined' && typeof res.result != 'undefined' && res.status == 'success') {
            if (res.result == 'connected') {
                $('#aws-unauthenticated-box').hide();
                $('#aws-authenticated-box').show();
                $('#bmip-aws-path').val(res.configs['path']).attr('readonly', true);
                $('#bmip-aws-bucket').val(res.configs['bucket']).attr('readonly', true);
                $('.bmi-dropdown[data-id="bmip-aws-storage-class"]').attr('data-selected', res.configs['storage-class']).val(res.configs['storage-class']).css('background', '#f1f1f1').css('pointerEvents', 'none');
                $('#storage-s3-row .region-container').hide();
                $('#storage-s3-row .aws-access-key-container').hide();
                $('#storage-s3-row .aws-secret-key-container').hide();
                $('#bmip-aws-sse').prop('checked', res.configs['sse'] === 'AES256').attr('disabled', true).css('cursor', 'not-allowed');
                $('#bmip-aws-sse').closest('.checkbox-container').css('cursor', 'not-allowed');
            } else {
                $('#aws-unauthenticated-box').show();
                $('#aws-authenticated-box').hide();
            }
  ...
  $('#bmi-pro-storage-aws-toggle').on('change', function (e) {
    e.preventDefault();
    $('#aws-authenticated-box').hide();
    $('#aws-unauthenticated-box').show();
    if ($('#bmi-pro-storage-aws-toggle').is(':checked') === true) {
      verifyAWSConnection();
    }
  });

  $('#wasabi-connect-btn').on('click', function (e) {
    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    let data = {};
    data['access-key'] = $('#bmip-wasabi-access-key').val();
    data['secret-key'] = $('#bmip-wasabi-secret-key').val();
    data['bucket'] = $('#bmip-wasabi-bucket').val();
    data['path'] = $('#bmip-wasabi-path').val();
    data['region'] = $('[data-id="bmip-wasabi-region"]').attr('data-selected');

    $.bmi.ajax('save-wasabi-config', data).then(function (res) {
      ...
      $.bmi.modal('freeze-loading-modal').close();
      $.bmi.alert('error', 'There was an error while trying to connect to Wasabi, please try again.');
    });
  });

  $('#wasabi-disconnect-btn').on('click', function (e) {

    e.preventDefault();
    $.bmi.modal('freeze-loading-modal').open();
    $.bmi.ajax('disconnect-wasabi').then(function (res) {
  ...
  function verifyWasabiConnection(silent = false) {
    $.bmi.ajax('verify-wasabi-connection', { uri: window.location.host}).then(function (res) {
        $('#wasabi-unauthenticated-box').show();
        $('#wasabi-authenticated-box').hide();


        if (typeof res.status != 'undefined' && typeof res.result != 'undefined' && res.status == 'success') {
            if (res.result == 'connected') {
                $('#wasabi-unauthenticated-box').hide();
                $('#wasabi-authenticated-box').show();
                $('#bmip-wasabi-path').val(res.configs['path']).attr('readonly', true);
                $('#bmip-wasabi-bucket').val(res.configs['bucket']).attr('readonly', true);
                $('#storage-wasabi-row .region-container').hide();
                $('#storage-wasabi-row .wasabi-access-key-container').hide();
                $('#storage-wasabi-row .wasabi-secret-key-container').hide();
            } else {
                $('#wasabi-unauthenticated-box').show();
                $('#wasabi-authenticated-box').hide();
            }
  ...
  $('#bmi-pro-storage-wasabi-toggle').on('change', function (e) {
      e.preventDefault();
      $('#wasabi-authenticated-box').hide();
      $('#wasabi-unauthenticated-box').show();
      if ($('#bmi-pro-storage-wasabi-toggle').is(':checked') === true) {
        verifyWasabiConnection();
      }
  });

  $('#bmi_restore_tbody').on('click', '.can-be-manually-uploaded', function (e) {

    e.preventDefault();
    const classList = $(this).attr('class').split(/\s+/);
    let type = false;
    let md5 = $(this).closest('tr').attr('md5');

    for (let cls of classList) {
      if (cls.startsWith('strg-')) {
        type = cls.replace('strg-', ''); // get the part after 'strg-'
        break;
      }
    }

    $.bmi.ajax('manually-enqueue-upload', {
      type: type,
      md5: md5
    }).then(function (res) {
      if (res.status == 'success') {
        $(this).removeClass('can-be-manually-uploaded');
        $.bmi.alert('success', 'Backup will be added to the upload queue shortly.');
      } else {
        $.bmi.alert('error', res.msg || 'There was an error while trying to enqueue the backup for upload.');
      }
    });

  });

  $(window).on("bmi-preload-collapsed", function (e) {

    const urlSearchParams = new URLSearchParams(window.location.search);
    const params = Object.fromEntries(urlSearchParams.entries());

    if (typeof params.bmi_drive != 'undefined') {
      window.history.pushState('', window.title, window.location.pathname + '?page=backup-migration');


        if (typeof params.bmi_auth_status != 'undefined' && params.bmi_auth_status == 'error') {
          if (typeof params.bmi_error_reason != 'undefined') {
            if (params.bmi_drive == 'gdrive') {
              if (params.bmi_error_reason == '1') return $.bmi.alert('error', 'Error during Google Drive connection: We could not setup auth space for you at this moment. [#GD-11].');
              if (params.bmi_error_reason == '2') return $.bmi.alert('error', 'Error during Google Drive connection: We could not check for existing authentication, please try again. [#GD-12].');
              if (params.bmi_error_reason == '3') return $.bmi.alert('error', 'Error during Google Drive connection: We could not update your your auth token, please try again. [#GD-13].');
              if (params.bmi_error_reason == '4') return $.bmi.alert('error', 'Error during Google Drive connection: Access has been denied by you during authorization. [#GD-14].');
              if (params.bmi_error_reason == '5') return $.bmi.alert('error', 'Error during Google Drive connection: Some unexpected error happened (google side), please try again. [#GD-15].');
              if (params.bmi_error_reason == '6') return $.bmi.alert('error', 'Error during Google Drive connection: Access token and refresh token has been revoked. [#GD-16].');
            } else if (params.bmi_drive == 'dropbox') {
              if (params.bmi_error_reason == '1') return $.bmi.alert('error', 'Error during Dropbox connection: We could not setup auth space for you at this moment. [#DB-11].');
              if (params.bmi_error_reason == '2') return $.bmi.alert('error', 'Error during Dropbox connection: We could not check for existing authentication, please try again. [#DB-12].');
              if (params.bmi_error_reason == '3') return $.bmi.alert('error', 'Error during Dropbox connection: We could not update your your auth token, please try again. [#DB-13].');
              if (params.bmi_error_reason == '4') return $.bmi.alert('error', 'Error during Dropbox connection: Access has been denied by you during authorization. [#DB-14].');
              if (params.bmi_error_reason == '5') return $.bmi.alert('error', 'Error during Dropbox connection: Some unexpected error happened (dropbox side), please try again. [#DB-15].');
              if (params.bmi_error_reason == '6') return $.bmi.alert('error', 'Error during Dropbox connection: Access token and refresh token has been revoked. [#DB-16].');
            }
          }
        }

        if (typeof params.bmi_token == 'undefined' || typeof params.bmi_secret == 'undefined' || typeof params.bmi_auth_status == 'undefined') {
          if (params.bmi_drive == 'gdrive') {
            return $.bmi.alert('error', 'Cannot save connection with Google Drive, due to insufficient details [#GD-09].');
          } else if (params.bmi_drive == 'dropbox') {
            return $.bmi.alert('error', 'Cannot save connection with Dropbox, due to insufficient details [#DB-09].');
          }
        }

        if (params.bmi_auth_status != 'success') {
          if (params.bmi_drive == 'gdrive') {
            return $.bmi.alert('error', 'Cannot save connection with Google Drive, due to unknown response from Google API [#GD-10].');
          } else if (params.bmi_drive == 'dropbox') {
            return $.bmi.alert('error', 'Cannot save connection with Dropbox, due to unknown response from Dropbox API [#DB-10].');
          }
        }

        $.bmi.modal('freeze-loading-modal').open();
        $('#bmi-external-storage-options').click();

        if (params.bmi_drive == 'gdrive') {
          if ($('#bmi-pro-storage-gdrive-toggle').is(':checked') === false) {
            $('#bmi-pro-storage-gdrive-toggle').click();
          }
        } else if (params.bmi_drive == 'dropbox') {
          if ($('#bmi-pro-storage-dropbox-toggle').is(':checked') === false) {
            $('#bmi-pro-storage-dropbox-toggle').click();
          }
        }

        if (params.bmi_drive == 'gdrive') {
          $.bmi.ajax('keep-gdrive-connection', { receivedToken: params.bmi_token, receivedClientID: params.bmi_secret }).then(function (res) {

            $.bmi.modal('freeze-loading-modal').close();

            setTimeout(function () {
              $('#bmi-pro-storage-gdrive-toggle')[0].scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });

              setTimeout(function () {
                $('#gdrive-unauthenticated-box').fadeOut();
                $('#gdrive-authenticated-box').fadeIn();
                setTimeout(function () {
                  $('#gdrive-authed-content-box')[0].scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
                  window.saveStorageWithoutClose = true;
                  $('#storage-options').find('.save-btn').click();
                }, 300);
              }, 300);
            }, 300);
            ...
            $.bmi.modal('freeze-loading-modal').close();
            $.bmi.alert('error', 'Cannot save connection with Google Drive, please try again [#GD-06].');

          });
        } else if (params.bmi_drive == 'dropbox') {
          $.bmi.ajax('keep-dropbox-connection', { receivedToken: params.bmi_token, receivedClientID: params.bmi_secret }).then(function (res) {

            $.bmi.modal('freeze-loading-modal').close();

            setTimeout(function () {
              $('#bmi-pro-storage-dropbox-toggle')[0].scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });

              setTimeout(function () {
                $('#dropbox-unauthenticated-box').fadeOut();
                $('#dropbox-authenticated-box').fadeIn();
                setTimeout(function () {
                  $('#dropbox-authed-content-box')[0].scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
                  window.saveStorageWithoutClose = true;
                  $('#storage-options').find('.save-btn').click();
                }, 300);
              }, 300);
            ...
            $.bmi.modal('freeze-loading-modal').close();
            $.bmi.alert('error', 'Cannot save connection with Dropbox, please try again [#DB-06].');

          });
        } else {
          jQuery(window).trigger("bmi-preload-collapsed-pro", [params]);
        }
    }


    if ($('#bmi-pro-storage-dropbox-toggle').is(':checked') === true) {
      verifyDropboxConnection(true);
    }

    if ($('#bmi-pro-storage-gdrive-toggle').is(':checked') === true) {
      verifyGDriveConnection(true);
    }

    if ($('#bmi-pro-storage-aws-toggle').is(':checked') === true) {
      verifyAWSConnection(true);
    }

    if ($('#bmi-pro-storage-wasabi-toggle').is(':checked') === true) {
        verifyWasabiConnection(true);
    }
  ...
  async function tryInDifferentWay() {
    ...
    if (errorParent == "backup-progress-modal") {
      ...
      $("#bmi-use-new-database-export-engine").prop("checked", true);
      await saveOtherOptionsPromise();
  ...
  function bmiSetFailureReasonsState(reasonCount) {
    const $plural = $(".there-are-reasons");
    const $single = $(".there-is-a-reason");
    const $none = $(".there-are-no-reasons");
    const $box = $(".debug-it-yourself");
  ...
  async function setupRestoreErrorOptions() {
    ...
    const tryInDifferentWayBtn = $(".try-in-different-way");
    const tryInDifferentWayOption = tryInDifferentWayBtn.closest(".failure-option");
    const failureReasons = $(".failure-reasons");

    if (
      !$("#cli-disable-others").is(":checked") ||
      $("#file_limit_extraction_max").val() != "300"
    ) {
      tryInDifferentWayOption.show();
    } else {
      tryInDifferentWayOption.hide();
    }
    ...
    try {
      ...
      if (res.status === "success" ) {
        if (Array.isArray(res.data) && res.data.length > 0) {
          res.data.forEach(r => failureReasons.append("<li>" + r + "</li>"));
          reasonCount = failureReasons.find("li").length;
        }
        if (res.mainReasonFound) {
          tryInDifferentWayOption.hide();
        }
      }
      bmiSetFailureReasonsState(reasonCount);
  ...
  async function setupBackupErrorOptions() {
    ...
    if (
      ...
      !$("#bmi-use-new-database-export-engine").is(":checked")
    ) {
      if (!tryInDifferentWayOption.hasClass("force_hide")) {
        tryInDifferentWayOption.show();
      }
    } else {
      tryInDifferentWayOption.hide();
  ...
  async function fillLogs(
    ...
    logPath = "?backup-migration=PROGRESS_LOGS&progress-id=latest_full.log&uncensored=true&bmi-id=current&t=" +
      +new Date() +
      "&sk=" +
      $("#BMI_SECRET_KEY").text().trim(),
  ) {
  ...
  function setupDownloadLogsHref(type = "backup") {
    ...
    let logs_url = url + "?backup-migration=PROGRESS_LOGS&progress-id=" + $progressId + "&bmi-id=current&t=" + +new Date() + "&sk=" + $("#BMI_SECRET_KEY").text().trim();
  ...
  function updateSuffixPosition() {
    let localBackupsPath = document.getElementById('bmi_path_storage_default');
    let localBackupsPathSuffix = document.getElementById('local-backups-suffix');
    let textWidth = getTextWidth(localBackupsPath.value, window.getComputedStyle(localBackupsPath).font);
    let suffixWidth = getTextWidth(localBackupsPathSuffix.innerText, window.getComputedStyle(localBackupsPathSuffix).font);

    localBackupsPath.style.paddingRight = suffixWidth + 22 + 'px';
    if (localBackupsPath.value === '') {
      localBackupsPathSuffix.style.display = 'none';
      return;
    } else {
      localBackupsPathSuffix.style.display = 'inline-block';
    }
    localBackupsPathSuffix.style.left = Math.min(textWidth + 20, localBackupsPath.clientWidth - suffixWidth - 20) + 'px';
  }
  ...
  $('#bmi_path_storage_default').on('input', updateSuffixPosition);
  $('#storage-options').on('click', function() {
    setTimeout(() => {
  ...
  $('#bmi_backup_search_input').on('input', function() {
    ...
    $.bmi.searchInBackups(searchTerm);
  ...
  $('#bmi_backup_search_input').on('keypress', function(e) {
    if (e.which === 13) {
      e.preventDefault();
      let searchTerm = $(this).val().toLowerCase();
      if (searchTerm.trim() === ''){
  ...
  $(window).on('resize', updateSuffixPosition);

  $(document).on('visibilitychange', async function() {
    if ((backupOnGoing || restoreOnGoing) && document.visibilityState === 'visible') {
      await $.bmi.requestWakeLock();
    }
  });

  $('#resync-with-ping-server').on('click', function(e) {
    e.preventDefault();
    $.bmi.ajax('resync-with-ping-server').then(function (res) {
      if (res.status == 'success') {
        $.bmi.alert('success', 'Your site has been registered successfully with our ping server.');
      } else {
        $.bmi.alert('error', res.msg || 'There was an error while trying to resync with ping server.');
      }
    }).catch(function (error) {
      $.bmi.alert('error', 'Cannot send request to your server, please check your internet connection.');
    });
  ...
  (function () {
    ...
    let logs_backup_uncensored =
      url_x +
      "?backup-migration=PROGRESS_LOGS&progress-id=latest.log&uncensored=true&bmi-id=current&t=" +
      +new Date() +
      "&sk=" +
      $("#BMI_SECRET_KEY").text().trim();
    let logs_restore =
      url_x +
      "?backup-migration=PROGRESS_LOGS&progress-id=latest_migration.log&bmi-id=current&t=" +
      +new Date() +
      "&sk=" +
      $("#BMI_SECRET_KEY").text().trim();
    let logs_staging =
      url_x +
      "?backup-migration=PROGRESS_LOGS&progress-id=latest_staging.log&bmi-id=current&t=" +
      +new Date() +
      "&sk=" +
      $("#BMI_SECRET_KEY").text().trim();
...
jQuery(document).ready(function($) {
  ...
  async function updateStagingLogs() {
    ...
    return new Promise(async function (resolve) {
      ...
      let res = await httpGet(url + '?backup-migration=PROGRESS_LOGS&progress-id=latest_staging_full.log&bmi-id=current&t=' + +new Date() + '&sk=' + $('#BMI_SECRET_KEY').text().trim());
