...
namespace ForminatorGoogleAddon\GuzzleHttp\Handler;

use ForminatorGoogleAddon\GuzzleHttp\Exception\ConnectException;
use ForminatorGoogleAddon\GuzzleHttp\Exception\RequestException;
use ForminatorGoogleAddon\GuzzleHttp\Promise as P;
use ForminatorGoogleAddon\GuzzleHttp\Promise\FulfilledPromise;
use ForminatorGoogleAddon\GuzzleHttp\Promise\PromiseInterface;
use ForminatorGoogleAddon\GuzzleHttp\Psr7\LazyOpenStream;
use ForminatorGoogleAddon\GuzzleHttp\TransferStats;
use ForminatorGoogleAddon\GuzzleHttp\Utils;
use ForminatorGoogleAddon\Psr\Http\Message\RequestInterface;
use ForminatorGoogleAddon\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \ForminatorGoogleAddon\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \ForminatorGoogleAddon\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \ForminatorGoogleAddon\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \ForminatorGoogleAddon\GuzzleHttp\Psr7\Utils::streamFor($sink);
